...
sortingRatingFieldsByEstelle = cloneDF(mergeRatings)
sortingRatingFieldsByEstelle = sortingRatingFieldsByEstelle.groupby(['movie_id', 'title'])['rating'].agg(
    COUNT=np.size, myAVG=lambda x: x.sum() / float(x.count())).sort_values('COUNT', ascending=False)
print('My info sorted: \n%s' % sortingRatingFieldsByEstelle[:15])
